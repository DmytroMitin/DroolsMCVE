...
object Main1 {
  ...
  def main(args: Array[String]): Unit = {
    println("rules must successfully trigger (activate)")
    ...
    println(sListener.getCount("throttle state activated")) //mustBe 1 // THIS TEST SUCCEEDS

    println("rules must successfully trigger (deactivate)")
    reset()
    (1 to 50).foreach(i => insertAndFire(mockMessage(offsetMSec = 200 * i), timeAdvanceMSec = 200))
    (1 to 60).foreach(_ => {
    ...
    println(sListener.getCount("throttle state activated")) //mustBe 1 // THIS TEST SUCCEEDS
    println(sListener.getCount("throttle state deactivated")) //c mustBe 1 // !!! THIS TEST FAILS !!!
  }
